...
MODEL_DIR = Path(__file__).resolve().parent.parent / "src" / "cheat_oracle" / "model"
...
PURE_MODULES = ("layers.py", "channels.py", "detectors.py", "predict.py")
...
def test_predict_imports_only_model_siblings() -> None:
    ...
    allowed = {".channels", ".detectors", ".layers", "dataclasses", "__future__"}
    seen: set[str] = set()
    for node in ast.walk(tree):
